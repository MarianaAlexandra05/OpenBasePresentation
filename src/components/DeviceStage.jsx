...
import { Mic, CheckCircle2, Play, RefreshCw, Terminal, Check, AlertTriangle, Radio, Sparkles, Laptop } from 'lucide-react';
  ...
  return (
    <section className="relative py-12 px-4 sm:px-6 max-w-5xl mx-auto">
      <div className="text-center mb-10">
        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-blue-500/10 border border-blue-500/20 text-sky-400 text-xs font-semibold uppercase tracking-wider mb-3">
          <Radio className="w-3.5 h-3.5 animate-pulse text-emerald-400" />
          <span>{t.deviceTitle}</span>
        </div>
        <h2 className="text-2xl sm:text-4xl font-extrabold text-white">
          Control Total desde el Móvil o tu Escritorio
        </h2>
      ...
      <div className="relative flex flex-col lg:flex-row items-center justify-center gap-8">
        ...
        {/* iPhone 16 Pro Frame */}
        <div className="iphone-frame shrink-0">
          <div className="iphone-screen">
            ...
            {/* In-App iOS Header */}
            <div className="flex items-center justify-between py-2 border-b border-white/[0.08] mb-3">
              <div className="flex items-center gap-2">
                <div className="w-6 h-6 rounded-lg bg-blue-600 flex items-center justify-center text-[10px] font-bold text-white">
                  OB
                </div>
                <div>
                  <div className="text-[10px] font-bold tracking-wider text-slate-400 uppercase">OPENBASE</div>
                  <div className="text-xs font-semibold text-white -mt-0.5">Voice Studio</div>
                </div>
              </div>
            ...
            <div className="flex items-center gap-2.5 p-2.5 rounded-xl bg-white/[0.04] border border-white/[0.06] mb-3">
              <div className="p-1.5 rounded-lg bg-sky-500/10 text-sky-400">
                <Laptop className="w-4 h-4" />
              </div>
              <div className="flex-1 min-w-0">
                <div className="text-xs font-semibold text-white truncate">{t.deviceConnected}</div>
                <div className="text-[10px] font-mono text-slate-400 truncate">{t.deviceRepo}</div>
              </div>
            ...
            <div className="text-center py-2">
              ...
              <h3 className="text-sm font-semibold text-white">{t.deviceListening}</h3>
              <p className="text-[11px] text-slate-400 mt-0.5 px-2">
            ...
            <div className="flex items-center justify-center gap-1 my-3 h-6">
              {[8, 16, 24, 12, 28, 18, 10, 22, 14, 20, 16, 8].map((height, i) => (
                <div
                  key={i}
            ...
            <div className="p-3 rounded-xl bg-slate-900/90 border border-white/[0.08] mb-3 shadow-inner">
            ...
            <div className="flex items-start gap-2.5 p-3 rounded-xl bg-blue-950/40 border border-blue-500/20 mb-3">
              ...
              <div className="text-xs">
                <div className="flex items-center gap-2 mb-0.5">
                  ...
                  <span className="text-[10px] text-slate-400">{t.deviceAssistantTime}</span>
                </div>
              ...
              {stage === 'running' ? (
                <div className="p-3 rounded-xl bg-amber-500/10 border border-amber-500/30">
                  <div className="flex items-center gap-2.5 mb-2">
                    ...
                    <div>
                      <div className="text-[10px] font-semibold text-amber-400 uppercase tracking-wider">{t.deviceApproved}</div>
                      <div className="text-xs font-bold text-white">{t.deviceRunningMsg}</div>
                    </div>
                  </div>
                ...
                <div className="p-3 rounded-xl bg-emerald-500/10 border border-emerald-500/30">
                  <div className="flex items-center justify-between mb-1.5">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <div className="text-[10px] font-bold text-emerald-400 uppercase tracking-wider">{t.deviceTestsPassed}</div>
                        <div className="text-xs font-semibold text-white">{t.deviceTestsPassedSuite}</div>
                      </div>
                    </div>
                    <span className="text-xs font-mono font-bold text-emerald-300">{t.deviceTestsCount}</span>
                  </div>
                ...
                <div className="p-3 rounded-xl bg-slate-900/90 border border-white/[0.1]">
                  <div className="mb-2">
                    <span className="text-[10px] font-bold text-amber-400 tracking-wider uppercase block">
                      {t.deviceApprovalNeeded}
                    </span>
                    <strong className="text-xs font-semibold text-white block mt-0.5">
                      {t.deviceApprovalTask}
                    </strong>
                  </div>
                  <code className="block text-[11px] font-mono bg-black/50 px-2 py-1 rounded text-sky-300 border border-white/[0.06] mb-3">
                    {currentPrompt.cmd}
                  </code>
                  <div className="flex gap-2">
                    <button
                      onClick={handleReset}
                      className="flex-1 py-1.5 rounded-lg bg-white/[0.08] hover:bg-white/[0.12] text-slate-300 text-xs font-semibold cursor-pointer transition-colors"
                    >
                      {t.deviceApprovalBtnReview}
                    </button>
                    <button
                      onClick={handleApprove}
                      className="flex-1 py-1.5 rounded-lg bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white text-xs font-bold shadow-lg shadow-emerald-500/25 flex items-center justify-center gap-1 cursor-pointer transition-all"
                    >
        ...
        {/* Side Controls & Real-time Scenario Switcher */}
        <div className="flex-1 max-w-md space-y-4 text-left">
          <div className="glass-card p-6 rounded-2xl border border-white/[0.08]">
            ...
            <p className="text-sm text-slate-400 mb-4">
              Prueba cómo reacciona el despachador autónomo ante diferentes comandos de ingeniería:
            </p>
              ...
              {prompts.map((p, idx) => (
                <button
                  ...
                  className={`w-full text-left p-3 rounded-xl text-xs transition-all cursor-pointer border ${
                    transcriptIndex === idx
                      ? 'bg-blue-600/20 border-sky-400/40 text-white shadow-md'
